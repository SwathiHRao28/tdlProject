...
\author{
\IEEEauthorblockN{1\textsuperscript{st} Swathi H Rao}
...
The proposed model improves consistently across all BLEU n-gram levels and ROUGE-L. The relative gain increases with n-gram order (BLEU-1: +0.95\% $\rightarrow$ BLEU-4: +2.49\%), indicating that the Counterfactual Loss suppresses hallucinated multi-word constructs by penalizing predictions that do not depend on specific visual patches.

CIDEr is effectively tied (0.4173 vs.\ 0.4176, $\Delta = -0.07\%$). CIDEr rewards captions resembling the average human reference; since COCO references tend to be generic scene descriptions, the model's gain in visual precision does not automatically translate into higher consensus scores. Critically, the absence of any CIDEr drop confirms that the explainability constraints impose no penalty on caption fluency or naturalness.
...
A custom comparison scanner (\texttt{compare\_models.py}) generated 4-panel visual matrices for 100 paired validation images. Each panel shows: (a) the input image with the Baseline caption and its cross-attention heatmap, (b) the input image with the Proposed caption and its attribution-aligned heatmap.

Of the 100 inspected pairs, \textbf{94 generated distinct captions}. Recurring patterns observed:
\begin{itemize}
    \item \textbf{Hallucination correction}: Baseline frequently predicted generic scene descriptors (``a truck driving on a road'', ``a clock on a wall'') when the actual object required fine-grained localization. The Proposed model's Counterfactual Loss disallowed coarse-grained patch usage, forcing token predictions to rely on specific spatial regions.
    \item \textbf{Attention focus}: Baseline cross-attention heatmaps were diffuse, spreading activation broadly across backgrounds. Proposed model heatmaps concentrated hot regions precisely on the entity being named, verified by higher correlation with gradient attribution maps.
\end{itemize}

% ─ Figure placeholder (replace with actual figures from comparisons_ablation.zip) ─
\begin{figure}[htbp]
\centerline{
% \includegraphics[width=\columnwidth]{figures/comparison_panel_1.png}
}
\caption{4-panel comparison: (left) Baseline caption + diffuse attention, (right) Proposed caption + localized attention heatmap. Proposed model correctly identifies the object and concentrates attention on the corresponding spatial patches.}
\label{fig:heatmap}
\end{figure}
